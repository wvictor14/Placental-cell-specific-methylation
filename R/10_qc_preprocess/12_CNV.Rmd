...
We are doing the Z-transformation workflow

https://academic.oup.com/bioinformatics/article/33/15/2266/3096425
https://rdrr.io/github/mknoll/cnAnalysis450k/f/vignettes/completeWorkflow.Rmd

First we define controls, samples with no known aneuploidys.

We don't really know if any sample has any CNVs or not. But we can assume most of the term 
samples
...
# controls
control_ind <- pDat %>% 
  filter(Sex == 'M', Trimester == 'Third', Tissue == 'Villi') %>% pull(Sample_Name)
cn_noob_ctrls <- cn_noob[,control_ind]

# query samples
cn_noob_samples <- cn_noob[,-which(colnames(cn_noob) %in% control_ind)]

# Z transform
cn_noob_ctrls <- scale(cn_noob_ctrls)
cn_noob_ctrls_med <- apply(cn_noob_ctrls, 1, 'median')

cn_noob_samples <- scale(cn_noob_samples)
```

## Find segments

```{r}
segments <- findSegments(data = cn_noob_samples[,1:5], 
                         ctrlAll = cn_noob_ctrls, ctrl = cn_noob_ctrls_med)
```

## Create CN matrix

```{r}
ctrlAll <- cn_noob_ctrls
segments_matrix <- createSegmentMatrix(segments, p.select = 0.05, arrayType = 'EPIC')
```
