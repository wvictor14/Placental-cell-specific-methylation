...
p <- stat_plot(gene_name = 'DNMT1', trimester = 'Third');p #h: 4.5, w:6.2
...
dmcs_filt <- dmcs %>%
  ...
  left_join(anno) %>%
  separate(Group1, into = c('Trimester', 'Celltype')) %>%
  mutate(Celltype = case_when(
    Celltype == 'Endo' ~'Endothelial',
    Celltype == 'Hofb' ~ 'Hofbauer',
    Celltype == 'Strom' ~ 'Stromal',
    Celltype == 'Troph' ~ 'Trophoblast'
  ))
...
p4 <- all_genes %>%
  ...
  scale_y_continuous(expand = c(0,0)) +
  labs(x = 'Gene-wise percentage of CpGs\nthat are differentially methylated', y = 'Count');p4 #h: 2.25, w:2.5
...
p_sd_pca <- pDat %>% 
  ...
  mutate(Tissue = ifelse(Tissue %in% c('Syncytiotrophoblast'),
                         Tissue, NA)) %>%
  ...
  guides(shape = guide_legend(override.aes = list(size = 1))) +
  labs(x = 'PC1 (41%)', y = 'PC2 (23%)', color = '') +
  coord_equal();p_sd_pca #h: 1.75, w: 2.75
...
p_std <- stb_troph_dens %>%
  ...
  labs(x = '', y = '', fill = '');p_std ###fig_height <- 3;fig_width <- 3.75
```
