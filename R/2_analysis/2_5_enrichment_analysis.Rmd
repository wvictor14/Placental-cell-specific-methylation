...
plot_enrich <- function(x) {
  ...
  facet_grid(Direction ~ Trimester , scales = 'free_y', space = 'free') +
...
#chromosome
background <- tibble(chr = paste0('chr', 1:22)) %>%
  mutate(xmin = c(0, seq(1.5, 21.5, 1)),
         xmax = c(seq(1.5, 21.5, 1), 23),
         col = rep(c('white','grey'), 11))
...
tests %>% 
  ...
  ggplot(aes(x = Celltype, y = genomic_feature, fill = Observed_p_minus_expected)) +
  geom_tile() +
  geom_point(aes(shape = bonferroni01)) +
  facet_grid(Trimester~Direction) +
  scale_shape_manual(values = c('TRUE' = '*', 'FALSE' = '')) +
  scale_fill_gradient2(low = '#1871ed', mid = 'white', high = '#f75c14', midpoint = 0) +
  coord_equal()
