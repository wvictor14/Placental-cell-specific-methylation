...
nrow(mqtls_filt) #928
length(unique(mqtls_filt$SNPID)) # 597
length(unique(mqtls_filt$cpgID)) # 928

# how many CpGs do we have?
sum(mqtls_filt$cpgID %in% rownames(betas_filt)) #722 out of 928
```

We have 722 cpgs out of 928 cpgs in our data. Why the discrepancy? Likely because 450k data was used for mqtls and some were removed in processing. 

Let's see if any were removed during processing

```{r}
# How many mQTL CpGs are in 850k data?
sum(mqtls_filt$cpgID %in% probe_anno$probe_ID) # 871 / 928

928-871 # 57 are exclusive to 450k
...
Actually a high number of these probes were filtered because they are cross hybridizing or have SNPs nearby (n=133). A small number were removed due to quality (n=23).
...
geno_df <- geno_df %>% filter(snp_name %in% mqtls_filt$SNPID)
nrow(geno_df) # 447 snps remain
597 - 447 # 150 snps removed

nrow(mqtls_filt) # 722 associations to test
```
...
results <- data %>%
  ...
  # apply model
  mutate(model = map(data, ~lm(betas~genotype+Sex, data = .)),
...
results %>%
  ...
  facet_wrap(~Significance, ncol = 1) +
  theme() +
  scale_y_continuous(limits = c(0, 800)) +
  labs(x = '', title = 'How many of 722 mQTLs', subtitle = 'were also mQTLs in our data')
