...
d <- runif(n = n, min = 0, max = 1-(c + b + a));d
e <- 1-(a+b+c+d);e
...
props %>%
  mutate(cpg = list(rownames(mset_combined)[1:10])) %>%
  unnest(cpg)
 
#### generate in silico mixtures
# filter to coefficient probes
mset_combined_coefs <- 
  mset_combined[c(rownames(coefs_combined_third), rownames(coefs_combined_first)),]

colnames(mset_combined_coefs) <- pData(mset_combined_coefs)$Sample_Name
betas_combined_coefs <- getBeta(mset_combined_coefs)
...
betas_combined_coefs %>%
  filter(cpg == "cg10590657",
         !Tissue %in% c('Villi', 'Mixture')) %>%
  group_by(Tissue) %>%
  sample_n(1)

betas_combined_coefs %>%
  filter(!Tissue %in% c('Villi', 'Mixture')) %>%
