...
houseman_estimates <- minfi:::projectCellType(
  getBeta(mset)[rownames(pl_cell_cpgs_third),pDat_villi$Sample_Name],
  pl_cell_cpgs_third,
  lessThanOne = FALSE)
...
# combine nrbc to placenta data
mset_combined_stb <- combineArrays(nrbc_noob, 
                               mset[,pData(mset)$CellType %in% c('Trophoblasts', 'Stromal', 
                                                                 'Hofbauer', 'Endothelial',
                                                                 'Syncytiotrophoblast')])
mset_combined_stb

pData(mset_combined_stb) <- pData(mset_combined_stb) %>% as.data.frame() %>%
  select(Sample_Name, Sex, Trimester, Tissue, Study) %>% 
  mutate(Tissue = gsub(' cs', '', Tissue),
         CellType = Tissue) %>% DataFrame()
  
pData(mset_combined_stb) 
pDat_combined_stb <- pData(mset_combined_stb) %>%
  as_tibble() 
  

# pick probes
probes_third_stb <- minfi:::pickCompProbes(
  mset_combined_stb[, pData(mset_combined_stb)$Trimester == 'Third'], 
  cellTypes = c('Trophoblasts', 'Stromal', 'Hofbauer', 'Endothelial', 'nRBC', 'Syncytiotrophoblast'),
  compositeCellType = 'Placenta',
  probeSelect = "both")

probes_first_stb <- minfi:::pickCompProbes(
  mset_combined_stb[, pData(mset_combined_stb)$Trimester == 'First' |
             pData(mset_combined_stb)$CellType %in% c('nRBC', 'Syncytiotrophoblast')], 
  cellTypes = c('Trophoblasts', 'Stromal', 'Hofbauer', 'Endothelial', 'nRBC', 'Syncytiotrophoblast'),
  compositeCellType = 'Placenta',
  probeSelect = "both")

# extract coefficients
coefs_combined_third_stb <- probes_third_stb$coefEsts
coefs_combined_first_stb <- probes_first_stb$coefEsts
```

## Heatmap

```{r}
pheatmap(coefs_combined_third_stb, cluster_cols = FALSE, cluster_rows = TRUE,
         show_rownames = FALSE) 
```
## apply to villi

```{r}
epidish_res_stb <- epidish(
  beta.m = getBeta(mset_test)[rownames(coefs_combined_third_stb)
                                ,pData(mset_test)$Trimester == 'Third'],
  ref.m = coefs_combined_third_stb,
  method = 'RPC')

epidish_res_stb_first <- epidish(
  beta.m = getBeta(mset_test)[rownames(coefs_combined_third_stb)
                                ,pData(mset_test)$Trimester == 'First'],
  ref.m = coefs_combined_third_stb,
  method = 'RPC')
...
# combine
epidish_res_stb <- epidish_res_stb$estF %>%
  as.data.frame() %>%
  bind_cols(Sample_Names = rownames(.), .) %>%
  pivot_longer(
    cols = -Sample_Names,
...
epidish_res_stb_first <- epidish_res_stb_first$estF %>%
  as.data.frame() %>%
  bind_cols(Sample_Names = rownames(.), .) %>%
  pivot_longer(
    cols = -Sample_Names,
...
epidish_rpc_stb <- bind_rows(epidish_res_stb, epidish_res_stb_first) %>%
  dplyr::rename(Sample_Name = Sample_Names) %>%
  left_join(pDat_filt %>% select(Sample_Name, Sex, Trimester, Tissue, GA))  %>%
  distinct() 
...
epidish_rpc_stb %>%
  ...
  ggplot(aes(x = Sample_Name, y = percent, fill = component)) +
  geom_bar(stat = 'identity') +
  scale_fill_manual(values = color_code_tissue[unique(epidish_rpc_stb$component)], 
                    na.value = '#636363') +
  theme(axis.text.x = element_blank(),
        axis.ticks.x = element_blank(),
        panel.border= element_blank(),
