...
dmcs_1vall_sig %>% 
  ...
  scale_y_continuous(limits = c(0, 20000),
                     ...
                     expand = c(0,0)) +
  scale_fill_manual(values = c('#6BAED6', '#DEEBF7'))
```
...
all_tested_cpgs <-  
  ...
  ## Is it an enhancer?
  rbind(data.frame(Var1 = c('Enhancer', 'Not Enhancer'), 
                   Freq = c(sum(anno$Phantom5_Enhancers != '', na.rm = T), 
                            sum(anno$Phantom5_Enhancers == '', na.rm =T)),
                   relation = 'Relation to Enhancers')) %>%
...
enrich_df <- dmcs_1vall %>% 
  ...
  mutate(UCSC_RefGene_Group = map2(anno, Group1_label, 
                                   ~as.data.frame(table(unlist(str_extract(
                                      .x$UCSC_RefGene_Group, "^[A-z0-9']+")))) %>%
                                    mutate(probeset = paste0(.y, ' DMCs (n=', nrow(.x), ')'),
                                           relation =  paste0('Relation to genes'))),
         Phantom5_Enhancers = map2(anno, Group1_label, 
                                   ~data.frame(Var1 = c('Enhancer', 'Not Enhancer'),
                                               Freq = c(sum(.x$Phantom5_Enhancers != "", na.rm = T),
                                                        sum(.x$Phantom5_Enhancers == '', na.rm = T)),
...
```{r eval = F}
dmcs_1vall %>% filter(bonferroni < 0.001, abs(estimate_b) > 0.5) %>% 
